...
YEAR_AGO = (datetime.datetime.now() - datetime.timedelta(days=365 * 2)).strftime("%Y-%m-%d")
...
- **Frequency**: частота запланованих покупок.
    ...
    - `D` - щодня.
- **Profit multiple**: поріг продажу відносно собівартості позиції.
    - поточна вартість позиції >= **profit_multiple** * базова вартість позиції.
- **Use fixed sell fraction**: увімкнути фіксовану частку продажу.
...
- **Invest limit**: кількість років для інвестування (коли інвестуєтся довше то поріг Profit multiple опускаєтся до 1)
...
- Якщо обрано Fixed sell fraction, то продаж відбувається на вказану частку позиції
- Умова take-profit перевіряється щодня через Profit multiple, але продаж може бути заблокований через cooldown після попереднього продажу.
- Фіксуються:
